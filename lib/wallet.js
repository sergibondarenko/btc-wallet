...
class BtcWallet {
  ...
  _createTransaction ({ fromAddr, toAddr, amount, privKey, fee }) {
    return this.info({address: fromAddr}).then((info) => {
      try {
        ...
        txb.sign(0, bitcoin.ECPair.fromWIF(privKey, this._network))
  ...
  sendCoins ({ fromAddr, privKey, toAddr, amount, fee = null } = {}) {
    if (!fromAddr || !toAddr || !amount || !privKey || fee == null) {
      return Promise.reject(new Error('transaction: fromAddr, toAddr, privKey, amount and fee must be specified'))
    }

    return this._createTransaction({ fromAddr, privKey, toAddr, amount, fee }).then((tx) => {
  ...
  create ({ mnemonic, childPath } = {}) {
    try {
      ...
      const address = {
        ...
        priv: this._getPrivAddress(child)
      }
